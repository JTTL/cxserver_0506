...
{
	//服务配置
	m_pMatchOption=PersonalRoomManagerParameter.pGameMatchOption;
	...
	m_pIDataBaseEngine=PersonalRoomManagerParameter.pICorrespondManager;
...
//比赛事件
bool CPersonalRoomGame::OnEventSocketMatch(WORD wSubCmdID, VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem, DWORD dwSocketID)
{
	switch (wSubCmdID)
	{
...
//进入事件
bool CPersonalRoomGame::OnEventEnterMatch(DWORD dwSocketID ,VOID* pData,DWORD dwUserIP, bool bIsMobile)
{
	//判断时间
...
//用户参赛
bool CPersonalRoomGame::OnEventUserJoinMatch(IServerUserItem * pIServerUserItem, BYTE cbReason,DWORD dwSocketID)
{
	//参数校验
	ASSERT(pIServerUserItem!=NULL);
	if (pIServerUserItem == NULL) return false;
...
//用户退赛
bool CPersonalRoomGame::OnEventUserQuitMatch(IServerUserItem * pIServerUserItem, BYTE cbReason, WORD *pBestRank, DWORD dwContextID)
{
	ASSERT(pIServerUserItem!=NULL);
	if (pIServerUserItem==NULL) return false;
...
//参赛权限
bool CPersonalRoomGame::VerifyUserEnterMatch(IServerUserItem * pIServerUserItem)
{
	ASSERT(pIServerUserItem!=NULL);
	if (pIServerUserItem==NULL) return false;
...
//截止报名
bool CPersonalRoomGame::IsMatchSignupEnd()
{
	//获取时间
	CTime CurTime=CTime::GetCurrentTime();
...
//开始报名
bool CPersonalRoomGame::IsMatchSignupStart()
{
	//获取时间
	CTime CurTime=CTime::GetCurrentTime();
...
//开始间隔
DWORD CPersonalRoomGame::GetMatchStartInterval()
{
	//获取时间
	CTime CurTime=CTime::GetCurrentTime();
	CTime MatchStartTime(m_pLockTimeMatch->MatchStartTime);
...
//執行起立
VOID CPersonalRoomGame::PerformAllUserStandUp(ITableFrame *pITableFrame)
{
	//参数校验
	ASSERT(pITableFrame!=NULL);
...
//计算场次
LONGLONG CPersonalRoomGame::CalcMatchNo()
{
	//循环开赛
	if (m_pLockTimeMatch->cbMatchMode==MATCH_MODE_ROUND)
	{
...
//分配用户
bool CPersonalRoomGame::PerformDistribute()
{
	//状态判断
...
//更新状态
VOID CPersonalRoomGame::UpdateMatchStatus()
{
	//变量定义
	CTime CurTime=CTime::GetCurrentTime();
	CTime MatchEndTime(m_pLockTimeMatch->MatchEndTime);
...
//切换时钟
VOID CPersonalRoomGame::SwitchMatchTimer()
{
	//比赛未开始
...
//插入用户
bool CPersonalRoomGame::InsertDistribute(IServerUserItem * pIServerUserItem)
{
	//效验参数
	ASSERT(pIServerUserItem!=NULL);
	if (pIServerUserItem==NULL) return false;
...
//清除玩家
void CPersonalRoomGame::ClearSameTableUser(DWORD dwUserID)
{
	//变量定义
...
//插入用户
void CPersonalRoomGame::InsertSameTableUser(DWORD dwUserID,DWORD dwTableUserID)
{
	//变量定义
	tagSameTableInfo * pSameTableInfo = m_DistributeManage.GetUserSameTableInfo(dwUserID);
	if(pSameTableInfo!=NULL)
...
//事件校验
bool CPersonalRoomGame::VerifyMatchTime(LPTSTR pszMessage,WORD wMaxCount,WORD & wMessageType)
{
	//变量定义
	CTime CurTime=CTime::GetCurrentTime();
	CTime MatchEndTime(m_pLockTimeMatch->MatchEndTime);
...
//发送用户分数
bool CPersonalRoomGame::SendMatchUserScore(IServerUserItem * pIServerUserItem)
{
	//发送数据
	CMD_GR_UserScore UserScore;
	ZeroMemory(&UserScore,sizeof(UserScore));
...
//发送分数
bool CPersonalRoomGame::SendMatchUserInitScore(IServerUserItem * pIServerUserItem)
{
	//获取成绩
	tagUserInfo *pUserScore=pIServerUserItem->GetUserInfo();
...
//复活提醒
bool CPersonalRoomGame::SendReviveNotify(IServerUserItem * pIServerUserItem, BYTE cbReviveTimesed, bool bUseSafeCard)
{
...
//保险提醒
bool CPersonalRoomGame::SendSafeCardNotify(IServerUserItem * pIServerUserItem)
{
	//构造结构
...
//报名提醒
bool CPersonalRoomGame::SendDeductFeeNotify(IServerUserItem * pIServerUserItem)
{
	//构造结构
...
//淘汰提醒
bool CPersonalRoomGame::SendEliminateNotify(IServerUserItem * pIServerUserItem)
{
	//变量定义
	TCHAR szMessage[128]=TEXT("");
	_sntprintf(szMessage,CountArray(szMessage),TEXT("由于您的比赛币低于 %d，您将不能继续游戏！"),m_pLockTimeMatch->lMatchCullScore);
...
//报名成功
bool CPersonalRoomGame::SendSignupSuccessNotify(IServerUserItem * pIServerUserItem)
{
	//报名成功
	if (m_MatchStatus!=MatchStatus_Free)
	{
...
//移除玩家
bool CPersonalRoomGame::RemoveDistribute(IServerUserItem * pIServerUserItem)
{
	//验证参数
	ASSERT(pIServerUserItem!=NULL);
	if (pIServerUserItem==NULL) return false;
...
//发送比赛信息
void CPersonalRoomGame::SendTableUserMatchInfo(ITableFrame *pITableFrame, WORD wChairID)
{
	//验证参数
	ASSERT(pITableFrame!=NULL);
	if (pITableFrame==NULL) return;
...
//写入奖励
bool CPersonalRoomGame::WriteUserAward(IServerUserItem *pIServerUserItem,tagMatchRankInfo * pMatchRankInfo)
{
	//参数效验
