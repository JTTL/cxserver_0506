...
bool CPersonalRoomGame::OnEventUserItemScore(IServerUserItem * pIServerUserItem,BYTE cbReason)
{
		...
		&&(pIServerUserItem->IsVariation()==true))
	{
		...
		sprintf(WritePersonalScore.szVideoNumber, "%s",  pIServerUserItem->GetVideoNumber());
...
//游戏结束
bool CPersonalRoomGame::OnEventGameEnd(WORD wTableID,  WORD wChairCount, DWORD dwDrawCountLimit, DWORD & dwPersonalPlayCount, int nSpecialInfoLen, byte * cbSpecialInfo, tagPersonalUserScoreInfo * PersonalUserScoreInfo)
{
...
bool CPersonalRoomGame::OnDBCreateSucess(DWORD dwContextID, VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	//诈金花房卡设置椅子个数
	if (m_pGameServiceAttrib->wKindID == ZJH_KIND_ID && ((m_pGameServiceOption->wServerType) & GAME_GENRE_PERSONAL) != 0)
	{
