...
bool CServerInfoManager::InsertGameServer(tagGameServerCreate * pGameServerCreate, tagGameServerInfo & GameServerResult)
{
	...
	try
	{
		...
		if (pGameServerCreate->wServerType == GAME_GENRE_PERSONAL)
		{
			...
			InsertPersonalCellScore(PlatformDBAide, pGameServerCreate->cbCustomRule,GameServerResult.wKindID);
		}
...
//插入私人房间参数
bool CServerInfoManager::InsertPersonalCellScore(CDataBaseAide PlatformDBAide,  byte * cbCustomRoom, WORD wKindID)
{
	try
	{
		tagDDZCustomRule DDZCustomRule;
		ZeroMemory(&DDZCustomRule, sizeof(DDZCustomRule));
		memcpy(&DDZCustomRule, cbCustomRoom, sizeof(DDZCustomRule));

		//执行命令
		PlatformDBAide.ResetParameter();
		PlatformDBAide.AddParameter(TEXT("@wKindID"),wKindID);
		PlatformDBAide.ExecuteProcess(TEXT("GSP_GR_DeletePersonalCellScore"),true);

		for(int i = 0;  i < MAX_CELL_SCORE; i++)
		{

			//插入参数
			PlatformDBAide.ResetParameter();
			PlatformDBAide.AddParameter(TEXT("@wKindID"),wKindID);
			PlatformDBAide.AddParameter(TEXT("@lCellScore"),DDZCustomRule.dwRoomCardCellScore[i]);
			...
			{
				//错误信息
				CTraceService::TraceString(TEXT("GSP_GR_InsertPersonalCellScore 插入底分配置出错"),TraceLevel_Exception);
			}
	...
	catch (IDataBaseException * pIException)
	{
		//错误信息
		LPCTSTR pszDescribe=pIException->GetExceptionDescribe();
		CTraceService::TraceString(TEXT("InsertPersonalCellScore 插入底分配置出错"),TraceLevel_Exception);
