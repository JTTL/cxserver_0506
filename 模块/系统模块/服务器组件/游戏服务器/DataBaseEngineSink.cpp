...
bool CDataBaseEngineSink::OnRequestWritePersonalGameScore(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	try
	{
		...
		if(CServerRule::IsSuportPlayBack(m_pGameServiceOption->dwServerRule))
		{
			...
			m_GameDBAide.AddParameter(TEXT("@strID"),pWriteGameScore->szVideoNumber);
			m_GameDBAide.AddParameter(TEXT("@strPersonalRoomGUID"), pWriteGameScore->szPersonalRoomGUID);
			m_GameDBAide.AddParameter(TEXT("@dwUserID"),pWriteGameScore->dwUserID);
			m_GameDBAide.AddParameter(TEXT("@szRoomID"),pWriteGameScore->szRoomID);
			m_GameDBAide.AddParameter(TEXT("@lScore"),pWriteGameScore->VariationInfo.lScore);
			m_GameDBAide.AddParameter(TEXT("@lGamesNum"),	pWriteGameScore->nGamesNum);

			lResultCode=m_GameDBAide.ExecuteProcess(TEXT("GSP_GR_WriteRecordBackInfo"),false);
		}
