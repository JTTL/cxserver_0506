...
BEGIN
	...
	BEGIN
		...
		IF @InsureScore = 0
		BEGIN
			SELECT  @InsureScore = InsureScore FROM RYTreasureDBLink.RYTreasureDB.dbo.GameScoreInfo WHERE UserID = @dwUserID
		END

		IF @Currency = 0
		BEGIN
			SELECT  @Currency = Currency FROM RYTreasureDBLink.RYTreasureDB.dbo.UserCurrencyInfo WHERE UserID = @dwUserID
		END
		...
		VALUES (@dwUserID, @lRoomCard, @dwItemCount, 0, 0, @cbType, @Currency, @InsureScore, '购买房卡', @strClientIP, GETDATE())
