...
BEGIN
	...
	declare @Code	INT
	SET @Code=0      
	select @Code=PlayBackCode from  [RYTreasureDB].[dbo].[RecordVideoPlayerInfo] where VideoNumber=@szVideoNumber and UserID=@dwUserID and DateDiff(d,BuildVideoTime,GetDate()) <= 1
	...
	BEGIN
		...
		BEGIN
			...
			IF NOT EXISTS (select * from  [RYTreasureDB].[dbo].[RecordVideoPlayerInfo] where PlayBackCode=@Code and DateDiff(d,BuildVideoTime,GetDate()) <= 1)
			BEGIN
				SET @bFind = 1  
				UPDATE [RYTreasureDB].[dbo].[RecordVideoPlayerInfo] SET PlayBackCode =@Code WHERE UserID=@dwUserID and VideoNumber=@szVideoNumber   
			END
		END 
	END	
